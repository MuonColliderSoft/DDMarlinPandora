/**
 *  @file   DDMarlinPandora/src/ParConeClusteringAlgorithm.cc
 * 
...
ParConeClusteringAlgorithm::ParConeClusteringAlgorithm() :
    m_trackNodes(),
    m_tracksKdTree(),
	m_hitNodes(),
	m_hitsKdTree(),
...
StatusCode ParConeClusteringAlgorithm::Run()
{
...
StatusCode ParConeClusteringAlgorithm::InitializeKDTrees(const TrackList *const pTrackList, const CaloHitList *const pCaloHitList)
{
...
StatusCode ParConeClusteringAlgorithm::SeedClustersWithTracks(const TrackList *const pTrackList, ClusterVector &clusterVector)
{
    if (0 == m_clusterSeedStrategy)
        return STATUS_CODE_SUCCESS;
...
StatusCode ParConeClusteringAlgorithm::GetCurrentClusterFitResults(const ClusterVector &clusterVector, ClusterFitResultMap &clusterFitResultMap) const
{
...
// inprogress
StatusCode ParConeClusteringAlgorithm::FindHitsInPreviousLayers(unsigned int pseudoLayer, const CaloHitVector &relevantCaloHits,
...
StatusCode ParConeClusteringAlgorithm::FindHitsInSameLayer(unsigned int pseudoLayer, const CaloHitVector &relevantCaloHits,
    const ClusterFitResultMap &clusterFitResultMap, ClusterVector &clusterVector)
{
    const float maxTrackSeedSeparation = std::sqrt(m_maxTrackSeedSeparation2);
...
StatusCode ParConeClusteringAlgorithm::GetGenericDistanceToHit(const Cluster *const pCluster, const CaloHit *const pCaloHit, const unsigned int searchLayer,
...
StatusCode ParConeClusteringAlgorithm::GetDistanceToHitInSameLayer(const CaloHit *const pCaloHit, const CaloHitList *const pCaloHitList,
...
StatusCode ParConeClusteringAlgorithm::GetConeApproachDistanceToHit(const CaloHit *const pCaloHit, const CaloHitList *const pCaloHitList,
...
StatusCode ParConeClusteringAlgorithm::GetConeApproachDistanceToHit(const CaloHit *const pCaloHit, const CartesianVector &clusterPosition,
    const CartesianVector &clusterDirection, float &distance) const
{
    const CartesianVector &hitPosition(pCaloHit->GetPositionVector());
...
StatusCode ParConeClusteringAlgorithm::GetDistanceToTrackSeed(const Cluster *const pCluster, const CaloHit *const pCaloHit, unsigned int searchLayer,
    float &distance) const
{
...
StatusCode ParConeClusteringAlgorithm::GetDistanceToTrackSeed(const Cluster *const pCluster, const CaloHit *const pCaloHit, float &distance) const
{
    const CartesianVector &hitPosition(pCaloHit->GetPositionVector());
...
StatusCode ParConeClusteringAlgorithm::RemoveEmptyClusters(const ClusterVector &clusterVector) const
{
...
StatusCode ParConeClusteringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
