...
MCMuonReconstructionAlgorithm::MCMuonReconstructionAlgorithm() :
...
StatusCode MCMuonReconstructionAlgorithm::Run()
{
...
StatusCode MCMuonReconstructionAlgorithm::AssociateMuonTracks(const ClusterList *const pMuonClusterList) const
{
...
StatusCode MCMuonReconstructionAlgorithm::GetMuonEntryPoint(const Helix &helix, const bool isPositiveZ, CartesianVector &muonEntryPoint) const
{
    const GeometryManager *const pGeometryManager(PandoraContentApi::GetGeometry(*this));
...
StatusCode MCMuonReconstructionAlgorithm::GetMuonCoilExitPoint(const Helix &helix, const bool isPositiveZ, CartesianVector &muonExitPoint) const
{
    const GeometryManager *const pGeometryManager(PandoraContentApi::GetGeometry(*this));
    const float coilMaxZ(std::fabs(pGeometryManager->GetSubDetector(COIL).GetOuterZCoordinate()));
...
const CartesianVector MCMuonReconstructionAlgorithm::getCentroidInMuonYoke(const Cluster *const pCluster) const
{
    const GeometryManager *const pGeometryManager(PandoraContentApi::GetGeometry(*this));
    const float muonEndCapInnerZ(std::fabs(pGeometryManager->GetSubDetector(MUON_ENDCAP).GetInnerZCoordinate()));
...
float MCMuonReconstructionAlgorithm::GetChi2OfTrackClusterFit(const Cluster *const pCluster, const Helix &helix) const
{
...
bool MCMuonReconstructionAlgorithm::PassTrackClusterCuts(const float &clusterFitChi2, const float &trackClusterChi2, const float &trackClusterD, float &trkTheta) const
{
...
StatusCode MCMuonReconstructionAlgorithm::AddCaloHits(const ClusterList *const pMuonClusterList) const
{
    const GeometryManager *const pGeometryManager(PandoraContentApi::GetGeometry(*this));
        ...
        clusterIter != clusterIterEnd; ++clusterIter)
    {
            ...
            layerIter != layerIterEnd; ++layerIter)
        {
            ...
            std::sort(trackDistanceInfoVector.begin(), trackDistanceInfoVector.end(), MCMuonReconstructionAlgorithm::SortByDistanceToTrack);
...
StatusCode MCMuonReconstructionAlgorithm::CreateMuonPfos(const ClusterList *const pMuonClusterList) const
{
    ...
    for (ClusterList::const_iterator iter = pMuonClusterList->begin(), iterEnd = pMuonClusterList->end(); iter != iterEnd; ++iter)
    {
        ...
        {
            std::cout << "MCMuonReconstructionAlgorithm: invalid/unexpected track relationships for muon." << std::endl;
            continue;
        }
...
StatusCode MCMuonReconstructionAlgorithm::TidyLists() const
{
...
StatusCode MCMuonReconstructionAlgorithm::GetPfoComponents(TrackList &pfoTrackList, CaloHitList &pfoCaloHitList, ClusterList &pfoClusterList) const
{
    const PfoList *pPfoList = NULL;
...
StatusCode MCMuonReconstructionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    // Input lists
